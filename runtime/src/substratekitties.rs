    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn breed_kitty(origin, kitty_id_1: T::Hash, kitty_id_2: T::Hash) -> Result{
            ...
            let new_kitty = Kitty {
                ...
                dna: final_dna,
                price: <T::Balance as As<u64>>::sa(0),
                gen: max(kitty_1.gen, kitty_2.gen) + 1,
            };
            Self::mint(sender, random_hash, new_kitty)?;
...
mod tests {
    ...
    use support::{impl_outer_origin, assert_ok, assert_err, assert_noop};
    ...
    fn owner_can_transfer() {
        with_externalities(&mut build_ext(), || {
            ...
            // retrieve newly created kitty
            let hash = Kitties::kitty_by_index(0);
            
            // transfer kitty from account 10 to 5
            assert_ok!(Kitties::transfer_from(10, 5, hash));
            ...
            assert_eq!(Kitties::owned_kitty_count(5), 1);

            // check that this kitty is specifically owned by account #10
            let hash = Kitties::kitty_by_index(0);
            assert_eq!(Kitties::owner_of(hash), Some(5));

            let other_hash = Kitties::kitty_of_owner_by_index((5, 0));
            assert_eq!(hash, other_hash);
        })
    }

    #[test]
    fn non_owner_cant_transfer() {
        with_externalities(&mut build_ext(), || {
            // create a kitty with account #10.
            assert_ok!(Kitties::create_kitty(Origin::signed(10)));
            ...
            // should not be able to transfer 5 since he doesn't own
            assert_err!(Kitties::transfer_from(5, 10, hash), "'from' account does not own this kitty");
